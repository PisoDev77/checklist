...
export default function Home() {
	...
	const fetchItems = async () => {
		try {
			const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/checkItems?date=${selectedDate}`);
	...
	const handleAddItem = async (e) => {
		...
		try {
			...
			const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/checkItems`, {
				method: 'POST',
				headers: {
	...
	const handleUpdateItem = async (updatedItem) => {
		try {
			...
			await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/checkItems/${updatedItem._id}`, itemToUpdate);
			fetchItems();
		} catch (error) {
			...
			<div className='max-w-2xl mx-auto'>
				<header className='flex flex-col md:flex-row justify-between mb-8'>
					...
					<section className='flex flex-col md:flex-row items-start md:items-center gap-2'>
						<Link href='/quiz' className='px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition duration-300'>
							퀴즈로 가기
						</Link>
						<section className='flex items-center gap-2'>
							...
							/>
							<button onClick={() => setSelectedDate(new Date().toISOString().split('T')[0])} className='px-4 py-2 bg-green-300 text-green-800 rounded-lg hover:bg-green-500 transition duration-300'>
								오늘
							</button>
						</section>
					</section>
				...
				<form onSubmit={handleAddItem} className='mb-8'>
					<div className='flex flex-col md:flex-row gap-2'>
						...
						/>
						<button type='submit' className='px-6 py-3 bg-green-500 text-white rounded-lg font-bold hover:bg-green-600 transition duration-300'>
							추가
						</button>
					</div>
				</form>
				...
				<div>
					{items.length === 0 ? (
						<div className='text-center py-10 bg-primary-50 rounded-lg text-green-800'>
							<p className='text-primary-800 text-lg mb-4'>{selectedDate === new Date().toISOString().split('T')[0] ? '오늘의 체크리스트가 없습니다' : '선택한 날짜의 체크리스트가 없습니다'}</p>
							<p className='text-primary-600'>위 폼을 통해 새로운 할 일을 추가해보세요!</p>
						</div>
					) : (
						<div className='space-y-2'>
							{items.map((item) => (
								<div key={item._id} className='flex flex-col gap-1'>
									<CheckListItem key={item._id} item={item} onUpdate={handleUpdateItem} onDelete={handleDeleteItem} />
									<div className='text-xs text-gray-500 ml-2'>
										<span>등록: {item.addDate ? new Date(item.addDate).toLocaleString('ko-KR') : '날짜 정보 없음'}</span>
										{item.recentUpdateDate && new Date(item.recentUpdateDate).toLocaleString('ko-KR') !== new Date(item.addDate).toLocaleString('ko-KR') && (
											<span className='ml-4'>최근 수정: {new Date(item.recentUpdateDate).toLocaleString('ko-KR')}</span>
										)}
									</div>
								</div>
							))}
